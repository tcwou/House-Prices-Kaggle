...
numeric_data = numeric_data.drop(columns=['SalePrice'])
ss = StandardScaler()
data[numeric_data.columns] = ss.fit_transform(data[numeric_data.columns])
test[numeric_data.columns] = ss.transform(test[numeric_data.columns])
...
EN_model = ElasticNet(normalize=False)
params = {'alpha': np.linspace(0.0003, 0.0008, 5),
          'l1_ratio': np.linspace(0.07, 0.8, 8)}
...
ordered_coefs = en_coefs.reindex(en_coefs.abs().sort_values(ascending = False).index)
print(ordered_coefs)
#End EN
